      ...
      <div className="p-6 w-full bg-base-100 rounded-xl shadow-md space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 sm:gap-4">
          ...
          <div className="flex justify-between">
            ...
            <span>{transformDateStr(userData?.created_at)}</span>
          </div>
              ...
              <table className="table w-full">
                <thead>
                  <tr>
                    <th>日期</th>
                    <th>支付方式</th>
                    <th className="min-w-20 text-center">状态</th>
                    <th>日志</th>
                  </tr>
                </thead>
                <tbody>
                  {[...userData.pay_log.logs].reverse().map((record, index) => (
                    <tr key={index}>
                      <td>{transformDateStr(record.date)}</td>
                      <td>{record.method}</td>
                      <td className="min-w-20 text-center">
                        <span
                          className={`badge ${record.is_success ? "badge-success" : "badge-error"} text-xs`}
                        >
