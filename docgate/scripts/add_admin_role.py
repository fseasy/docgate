...
from docgate.supertokens_utils import add_role2user, get_user_by_email


def add_admin_for_emails(emails: Iterable[str]) -> None:
  init_supertokens()
  ...
  for email in emails:
    ...
    for user in users:
      ...
      try:
        result_info = add_role2user(user_id, StRole.ADMIN)
        if result_info:
...
def main():
  ...
  add_admin_for_emails(args.email)
